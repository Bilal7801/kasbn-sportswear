{{-- register.blade.php ── Fully Fixed Variant matching Login --}}
@extends('layouts.app')

@section('title', 'Create Account — SialkotPro Sports')

@push('styles')
<style>
    /* ── SCOPED REGISTER LAYOUT CONTAINER ── */
    ...
    .register-scan-line {
        ...
        animation: registerLineScan 8s cubic-bezier(0.4, 0, 0.2, 1) infinite;
        pointer-events: none;
    }

    @keyframes registerLineScan { 0% { top: -10%; } 100% { top: 110%; } }
    ...
    .register-card {
        ...
        border-top: 4px solid #000000; /* Uses your primary style accent to match login */
        border-radius: 8px; 
        padding: 35px 28px;
        box-shadow: 0 12px 40px rgba(0, 0, 0, 0.05);
    }

    /* Moved back-link to match login structure safely */
    .back-link-wrapper {
        margin-bottom: 14px;
        display: block;
    }
    ...
    .register-title { font-family: var(--font-display); font-size: 36px; font-weight: 400; text-transform: uppercase; letter-spacing: 0.5px; color: #000000; margin-bottom: 6px; line-height: 1; }
    .register-sub { font-family: var(--font-body); font-size: 13px; color: #6b7280; margin-bottom: 24px; line-height: 1.4; }
    ...
    .form-input.is-invalid { border-color: #ef4444; background: #fffbfb; }
    .form-input.is-invalid + .input-icon { color: #ef4444; }
    ...
    .terms-row { margin-bottom: 20px; width: 100%; }
    .terms-label { display: flex; align-items: center; gap: 6px; font-family: var(--font-body); font-size: 12px; color: #4b5563; cursor: pointer; user-select: none; }
    ...
    .login-row { margin-top: 24px; text-align: center; font-family: var(--font-body); font-size: 13px; color: #6b7280; }
    .login-row a { font-family: var(--font-cond); font-weight: 700; letter-spacing: 0.5px; text-transform: uppercase; color: #000000; text-decoration: none; margin-left: 3px; }
    .login-row a:hover { color: var(--gold-light); }
...
<div class="register-page-wrapper">
    <div class="register-body-grid"></div>
    <div class="register-scan-line"></div>

    <div class="register-container fade-up">
        <div class="back-link-wrapper">
            <a href="{{ route('home') }}" class="back-link"><i class="fas fa-arrow-left"></i> Back to Site</a>
        </div>
            ...
            <form id="registerForm" method="POST" action="{{ route('register') }}" autocomplete="off">
                ...
                <div class="input-group">
                    ...
                    <div class="input-wrapper">
                        <input type="text" name="name" class="form-input @error('name') is-invalid @enderror" value="{{ old('name') }}" placeholder="Enter full name" required autofocus autocomplete="off">
                ...
                <div class="input-group">
                    <label class="input-label">Communication Endpoint</label>
                    <div class="input-wrapper">
                        <input type="email" name="email" class="form-input @error('email') is-invalid @enderror" value="{{ old('email') }}" placeholder="Enter email address" required autocomplete="off">
                ...
                <div class="input-group">
                    <label class="input-label">Access Token Code</label>
                    <div class="input-wrapper">
                        <input type="password" name="password" id="password" class="form-input @error('password') is-invalid @enderror" placeholder="Create password" required autocomplete="off">
                ...
                <div class="input-group">
                    <label class="input-label">Confirm Token Code</label>
                    <div class="input-wrapper">
                        <input type="password" name="password_confirmation" id="password_confirmation" class="form-input" placeholder="Confirm password" required autocomplete="off">
                ...
                <div class="terms-row">
                    <label class="terms-label @if(old('terms')) checked @endif" id="termsLabel">
                        <input type="checkbox" name="terms" id="termsChk" value="1" required @if(old('terms')) checked @endif>
...
@push('scripts')
<script>
    // Advanced password toggle structural loop handler
    ...
    // Clean inline interaction trigger for checkbox custom container layouts
    const termsLabel = document.getElementById('termsLabel');
    const termsChk = document.getElementById('termsChk');
    
    if (termsChk && termsLabel) {
        termsChk.addEventListener('change', (e) => {
            termsLabel.classList.toggle('checked', e.target.checked);
        });
    }
</script>
@endpush